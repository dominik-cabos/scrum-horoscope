...
export async function GET(request: NextRequest) {
    try {
        ...
        const horoscope = getHoroscopeById(parseInt(horoscopeId));
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Validate input
        if (!requestData.firstName || !requestData.birthday || !requestData.photoUrl || !requestData.occupation) {
            return NextResponse.json(
